...
typeOfStmt : TYPEOF WS valueStmt (WS IS WS aType)?;
...
withStmt : 
	WITH WS (implicitCallStmt_InStmt | (NEW WS aType)) endOfStatement
	block? 
	END_WITH
;
...
asTypeClause : AS WS? (NEW WS)? aType (WS? fieldLength)?;
